...
62, 76, 36 },
			...
			{ 20, 73, 35, 29, 78, 31, 90, 01, 74, 31, 49, 71, 48, 86, 81, 16,
					23, 57, 05, 54 },
			{ 1, 70, 54, 71, 83, 51, 54, 69, 16, 92, 33, 48, 61, 43, 52, 01,
					89, 19, 67, 48 } };

	static int traverseGridDown() {
...
valOne = grid[i][j];
		...
		//		System.out.println("Column " + j + ": " + valOne + " " + valTwo + " " + valThree + " " + valFour);
				int currentProduct = valOne * valTwo * valThree * valFour;
...
//		
System.out.println("Row " + j + ": " + valOne + " " + valTwo + " " + valThree + " " + valFour);
				int currentProduct = valOne * valTwo * valThree * valFour;
				if (currentProduct > greatestProduct)
					greatestProduct = currentProduct;
	  ...
	  static int traverseGridDiagonallyLeftCorner() 
	  {
			int greatestProduct = 1;
			int valOne = 0;
			int valTwo = 0;
...
for (int j = 0; j < 17; j++)
 ...
 valOne = grid[j][i];
			  valTwo = grid[j + 1][i +1];
			  valThree = grid[j + 2][i+2];
			  valFour = grid[j + 3][i + 3];
	
//	
System.out.println("Vals " + ": " + valOne + " " + valTwo + " " + valThree + " " + valFour);
	  ...
	  static int traverseGridDiagonallyRightCorner() 
	  {
			int greatestProduct = 1;
			int valOne = 0;
...
int valTwo = 0;
			...
			int valFour = 0;
	  //first row logic 
		for (int j = 0; j < 17; j++)//row
	
{
	
 for (int i = 19; i >= 3; i--)//column
		  {
...
System.out.println("Vals " + ": " + valOne + " " + valTwo + " " + valThree + " " + valFour);
...

 {
	  ...
	  int currentGreatest = traverseGridDown();
		  int competitor = traverseGridRight();
		  if (competitor > currentGreatest)
			  currentGreatest = competitor;
	
 competitor = traverseGridDiagonallyLeftCorner();
		  if (competitor > currentGreatest)
			  currentGreatest = competitor;
	
 competitor = traverseGridDiagonallyRightCorner();
		  if (competitor > currentGreatest)
			  currentGreatest = competitor;
